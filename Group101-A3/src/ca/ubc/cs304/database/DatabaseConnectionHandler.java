...
public class DatabaseConnectionHandler {
    ...
    public int viewtheNumofAvaliableVehicle(String vtname, String fromDateTime, String toDateTime, String location, String city) {
        int num = 0;
        try {
            PreparedStatement ps = connection.prepareStatement("WITH bad(revn) AS( " +
                    "SELECT COUNT(*) " +
                    "FROM RESERVATIONS r1 " +
                    "WHERE  ((TO_DATE(NVL(?,'31-DEC-9999 23:59'),'DD-MON-YYYY HH24:MI') BETWEEN r1.fromDateTime AND r1.toDateTime) " +
                    "OR (TO_DATE(NVL(?,'31-DEC-9999 23:59'),'DD-MON-YYYY HH24:MI') BETWEEN r1.fromDateTime AND r1.toDateTime))" +
                    "AND (r1.vtname=? OR ? is null) AND ((r1.location=? OR ? is null )) AND ((r1.city=? OR ? is null )))" +
                    " SELECT g.n,b.revn, g.n-b.revn AS res FROM bad b,(SELECT COUNT(*) AS n FROM Vehicle v WHERE (v.vtname=? OR ? is null ) AND (v.location=? OR ? is null ) AND (v.CITY=? OR ? is null )) g");
            if (vtname == null) {
...
//            ps.setInt(1, confNo);
    ...
    public ArrayList<VehicleModel> DRentalB(String location, String city, String day) {
        ...
        try {
            PreparedStatement ps = connection.prepareStatement("SELECT * FROM VEHICLE v " +
    ...
    public ArrayList<Model[]> DReturnB(String location, String city, String day) {
        ...
        System.out.println(day);
        try {
            PreparedStatement ps = connection.prepareStatement("SELECT * FROM VEHICLE v,RETURN re" +
                    " WHERE v.LOCATION=? AND v.city=?" +
    ...
    public Boolean checkforCustomer(String dlicense) {
        Boolean ret = false;
        try {
            PreparedStatement psC = connection.prepareStatement("SELECT * FROM CUSTOMER c WHERE c.DLICENSE=?");
            psC.setString(1, dlicense);
            psC.executeUpdate();
...
return ret;
    ...
    public Boolean makeReservation(ReservationModel rse) {
        ...
        try {
            PreparedStatement psC = connection.prepareStatement("SELECT * FROM CUSTOMER c WHERE c.DLICENSE=?");
            psC.setString(1, rse.getDlicense());
            psC.executeUpdate();

            ResultSet rs = psC.executeQuery();
            if (rs.next()) {ret=true;psC.close(); return ret;}
            psC.close();


            PreparedStatement ps = connection.prepareStatement("INSERT INTO Reservations VALUES (?,?,?,TO_DATE(NVL(?,'01-JAN-9999 00:00'),'DD-MON-YYYY HH24:MI'),TO_DATE(NVL(?,'01-JAN-9999 00:00'),'DD-MON-YYYY HH24:MI'))");
            ps.setInt(1, rse.getConfno());


            if (rse.getVtname() == null) {
                ps.setNull(2, Types.VARCHAR);
            } else {
                ps.setString(2, rse.getVtname());
            }

            if (rse.getDlicense() == null) {
                ps.setNull(3, Types.VARCHAR);
            } else  ps.setString(3, rse.getDlicense());

            if (rse.getFromDateTime()== null) {
                ps.setNull(4, Types.VARCHAR);
            } else ps.setString(4, rse.getFromDateTime());

            if (rse.getToDateTime()== null) {
                ps.setNull(5, Types.VARCHAR);
            } else ps.setString(5, rse.getToDateTime());
            ...
            connection.commit();
            ps.close();
           // System.out.println("You've made a reservation for " + fromDateTime + " to " + toDateTime);
//            if (vtname != null) {
